# InSpec test for recipe scrapyd-deploy::scrapyd
...
denpendencies_upgrade_path = File.join(conf_dir, 'dependencies_upgrade.sh')
...
describe file(denpendencies_upgrade_path) do
  it { should exist }
  its('owner') { should eq 'root' }
  its('group') { should eq 'root' }
  its('mode') { should eq 493 }
  its('content') { should match 'devpi' }
end
...
describe crontab('root').commands(denpendencies_upgrade_path) do
